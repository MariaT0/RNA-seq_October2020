...
### Annotate our heatmap (optional)
annotation <- data.frame(analysis.groups=sample_guide[,'analysis.groups'], 
                     row.names=rownames(sample_guide))
...
Following the steps on this website http://environmentalcomputing.net/heat-maps/?fbclid=IwAR2-6wdTdeK0lModZhbD_C6nNy-K5QCuSeOhYeKWxbnT_bTM47yrhC_nf8k

1. I want my counts data to have row names (genes)
```{r}
# use first column for row names
sig_DE_lncRNA_hepg2_counts <- data.frame(sig_DE_lncRNA_hepg2_counts, row.names = 1)
```

2. coloring sample groups

Before looking further at how clustering effects the patterns observed, we should add some colours associated with the treatment groups next. The simplest method is to use the sample_guide dataframe as input, taking care that you 1) specify categorical covariates (factor groups) , 2) that row names in Bac.factors match those in Bac.counts, and 3) then remove the factors you do not want to show.
...
#make analysis groups a factor

sample_guide$analysis.groups<-as.factor(sample_guide$analysis.groups)

#select the labels I care about

sample_guideLabels <- select(sample_guide, sample.ID, analysis.groups)

#get rid of NA values in sample_guide labels

na.omit(sample_guideLabels)

#plotting the heatmap

pheatmap(sig_DE_lncRNA_hepg2_counts, annotation_col = sample_guideLabels)
```

same error. perhaps need to transpose columns/rows
...
# get data
  data("sample_guideLabels")

# transpose
  t_sample_guideLabels <- transpose(sample_guideLabels)

# get row and colnames in order
  colnames(t_sample_guideLabels) <- rownames(sample_guideLabels)
  rownames(t_sample_guideLabels) <- colnames(sample_guideLabels)
  print(t_sample_guideLabels)
```
Now try again
...
pheatmap(sig_DE_lncRNA_hepg2_counts, annotation_col = t_sample_guideLabels)

```

I'm goin to back and try again with my new labels

```{r}


### Annotate our heatmap (optional)
annotation <- data.frame(t_sample_guideLabels)
...
```{r}
str(t_sample_guideLabels)
```

```{r}
### Run pheatmap
pheatmap(sig_DE_lncRNA_hepg2_counts, color = heat.colors, cluster_rows = T, show_rownames=F,
 border_color=NA, fontsize = 10, scale="row",
     fontsize_row = 10, height=20)
```
```{r}
my_sample_col<-data.frame(sampleType=rep("analysis.groups"), )
row.names(my_sample_col) <- colnames(sample_guide)
```
my_sample_col <- data.frame(sample = rep(c("tumour", "normal"), c(4,2)))
row.names(my_sample_col) <- colnames(data_subset)

```{r}
remove(my_sample_col)
remove(sample_guideHepG2)
...
Bac.factorsDS <- select(melted_sig_DE_lncRNA_hepg2_counts,sample.ID,analysis.groups)
Labels<-Bac.factorsDS
na.omit(Labels)

pheatmap(sig_DE_lncRNA_hepg2_counts, annotation_col = Labels)
```
