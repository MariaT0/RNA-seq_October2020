...
Note that this includes samples for HepG2 and Huh7, yet these are the lncRNAs that are differentially expressed in HepG2. 

```{r}
### Set a color palette
 heat.colors <- brewer.pal(11, "Set1")
### Run pheatmap
pheatmap(sig_DE_lncRNA_hepg2_counts, color = heat.colors, cluster_rows = T, show_rownames=T,
...
```{r}
sigHepG2lncs_counts <- t_sigHepG2counts[HepG2samples$sample.ID %in% t_sigHepG2counts$sample.ID,]
remove(sigHepG2lncs_counts)
```
...
heat.colors <- brewer.pal(8, "Blues")
